...
class DocumentParser {
    ...
    async parseQuestionsDocument(fileBuffer) {
        try {
            ...
            const imageMap = new Map(); // placeholder id → base64 data URI
            let imageCounter = 0;
                ...
                { buffer: fileBuffer },
                {
                    convertImage: mammoth.images.imgElement(async (image) => {
                        try {
                            ...
                            // Return placeholder text so it appears in extracted text
            ...
            // Build an ordered list of image placeholders from the HTML so we know
            // the position of each image relative to the surrounding text
            const htmlContent = htmlResult.value;
            const imagePlaceholders = []; // { placeholder, index in html }
            ...
            // We'll attach images to questions after parsing using html paragraph positions
            // Build a simplified paragraph list from the html for image association
            ...
            for (let i = 0; i < allLines.length; i++) {
                ...
                if (!line) continue; // skip empty lines (from image paragraphs)
            ...
            // --- Step 4: Assign images to questions using HTML paragraph proximity ---
            // Strategy: distribute images to questions in document order.
            // Each image is assigned to the question whose rawText appears nearest
            // before the image in the html paragraph list.
            if (imageMap.size > 0) {
                // Build question → html-paragraph index map by matching rawText
                ...
                imagePlaceholders.forEach(({ placeholder }) => {
                    ...
                    // Find which paragraph in htmlParagraphs contains this placeholder
                    const paraIdx = htmlParagraphs.findIndex(p => p.includes(placeholder));

                    // Find the question whose paragraph index is closest but <= paraIdx
                    let bestQ = null;
                    let bestDiff = Infinity;
                    questions.forEach((q, qi) => {
                        const qParaIdx = questionParagraphIndex[qi];
                        const diff = paraIdx - qParaIdx;
                    ...
                    if (bestQ) {
                        if (!bestQ.images) bestQ.images = [];
                        bestQ.images.push(dataUri);
                        console.log(`🖼️  Assigned image to question ${bestQ.id}`);
                    } else if (questions.length > 0) {
                        // Fallback: attach to last question
                        const lastQ = questions[questions.length - 1];
                        if (!lastQ.images) lastQ.images = [];
                        lastQ.images.push(dataUri);
                    }
                });

                console.log(`🖼️  Image assignment complete. Questions with images: ${questions.filter(q => q.images && q.images.length > 0).length}`);
            }
    ...
    processEnhancedQuestionLines(question, lines) {
        ...
        for (const line of lines) {
            ...
            // Not a question line and not anything else — treat as a bare list item
            // (Word auto-lettered lists lose their A/B/C/D prefix in raw text extraction)
                ...
                if (nextLetter && question.options.length < 6) {
                    // Only auto-assign if we haven't started getting explicit lettered options
                    // OR if existing options were also auto-assigned
                    ...
                    if (!hasExplicitOptions) {
                        question.options.push({
                            ...
                            text: trimmedLine,
                            isCorrect: false,
                            explicit: false  // flag: came from auto-assignment
                        });
    ...
    processEnhancedOptionLine(question, line) {
        ...
        question.options.push({
            ...
            explicit: true    // This option had an explicit A/B/C/D letter prefix
        });
    ...
    async generateResultsDocument(results, examTitle = 'Army Examination Results') {
        try {
            ...
            // Recalculate grades for all results (in case they were calculated with old PASS/FAIL logic)
